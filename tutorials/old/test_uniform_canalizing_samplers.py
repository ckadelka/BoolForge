...
for _ in range(nsim):
    ...
    res.append(boolforge.utils.bin2dec(bf))
print(pd.Series(res).value_counts())

A = pd.Series(res).value_counts()
A = pd.DataFrame(A)
A['f'] = [boolforge.utils.dec2bin(el,2**n) for el in A.index]
A['layer_structure'] = [str(boolforge.BooleanFunction(f).get_layer_structure()['LayerStructure']) for f in A['f']]
A['w'] = [min(sum(boolforge.utils.dec2bin(el,2**n)), 2**n - sum(boolforge.utils.dec2bin(el,2**n))) for el in A.index]
