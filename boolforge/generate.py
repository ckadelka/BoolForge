...
def random_function(n,k=0, EXACT_DEPTH=False, layer_structure=None, 
    ...
    if LINEAR:
    ...
    elif k>0 and layer_structure is None:
        return random_k_canalizing_function(n, min(k, n), EXACT_DEPTH=EXACT_DEPTH, ALLOW_DEGENERATED_FUNCTIONS=ALLOW_DEGENERATED_FUNCTIONS)
        ...
        if ALLOW_DEGENERATED_FUNCTIONS:
            ...
            else: #completely random function, generated by default
                return BooleanFunction(np.array(np.random.random(2**n) < bias_of_function, dtype=int))
        else:
            if EXACT_DEPTH is True:
        ...
        assert type(hamming_weight)==int and 0<=hamming_weight<=2**n, "Hamming weight must be an integer in {0,1,...,2^n}"
        assert 1<hamming_weight<2**n-1 or not EXACT_DEPTH,"If EXACT_DEPTH==True and k==0, Hamming_weight must be in 2,3,...,2^n-2. All functions with Hamming weight 0,1,2^n-1,2^n are canalizing"
...
def random_non_degenerated_function(n, bias=0.5):
    ...
    assert type(bias)==float and 0.001<bias<0.999,"almost all extremely biased Boolean functions are degenerated. Choose a more balanced value for the 'bias'."
    while True:  # works well because most Boolean functions are non-degenerated
        f = random_function(n, bias=bias)
        if not f.is_degenerated():
            return f
...
def random_degenerated_function(n, bias=0.5):
    ...
    f_original = random_function(n-1, bias=bias)
...
def random_k_canalizing_function(n, k, EXACT_DEPTH=False, ALLOW_DEGENERATED_FUNCTIONS=True):
    ...
    if k < n:
        core_polynomial = random_function(n=n-k,k=0,EXACT_DEPTH=EXACT_DEPTH,ALLOW_DEGENERATED_FUNCTIONS=ALLOW_DEGENERATED_FUNCTIONS)
    else:
...
def random_network(N=None, n=None, 
    ...
    F = [random_function(indegrees[i], k=k, EXACT_DEPTH=EXACT_DEPTH, layer_structure=layer_structure, 
                     ...
                     hamming_weight=hamming_weight) for i in range(N)]
