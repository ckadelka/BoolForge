...
def get_dimension_trap_space(points):
    ref = points[0]
    varying = 0
    for s in points[1:]:
        varying |= (ref ^ s)
...
class BooleanNetworkRobustnessMixin:
    def get_terminal_sccs_and_robustness_asynchronous_exact(self) -> dict:
        ...
        hardware).
Each terminal SCC represented as a list of decimal states.
        Returns
        -------
        dict
            ...
            - DimensionTrapSpaces : np.ndarray of int
        ...
        dim_trap_spaces = np.array(list(map(get_dimension_trap_space,terminal_sccs)))
